...
///
/// 包含以下通知页面：
/// - LikesAndFavoritesScreen  赞和收藏
/// - NewFollowersScreen       新增关注
/// - CommentsAndMentionsScreen 评论和@
/// - DiscoverScreen           发现（占位）
///
/// 最终归属 P4 interaction agent。
...
class _LikesAndFavoritesScreenState extends State<LikesAndFavoritesScreen> {
  ...
  Future<void> _loadNotifications({bool loadMore = false}) async {
    ...
    try {
      final resp =
          await ApiService.getLikesAndFavorites(page: _page, pageSize: _pageSize);
  ...
  void _openUserProfile(String userId) {
    ...
    Navigator.push(
        context, MaterialPageRoute(builder: (_) => ProfilePage(userId: userId)));
  }
...
class _NewFollowersScreenState extends State<NewFollowersScreen> {
  ...
  Future<bool> _isActorInMyFollowing({
    ...
    while (true) {
      try {
        final resp = await ApiService.getFollowing(currentUserId,
        ...
        final found = users.any((userJson) {
          final id =
              (userJson['id'] ?? userJson['userId'])?.toString() ?? '';
            ...
            if (notification.post != null) ...[
              ...
              Container(
                ...
                child: Row(children: [
                  ...
                  const SizedBox(width: 8),
                  Expanded(
                    child: Text(notification.post!.title,
                        style: TextStyle(color: secondary, fontSize: 13),
                        maxLines: 1,
                        overflow: TextOverflow.ellipsis),
                  ),
